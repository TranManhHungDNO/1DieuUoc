 ...
 * @Author PA SOFTWARE SOLTIONS (contact@pasvietnam.vn)
 * @Copyright (C) 2015 PA SOFTWARE SOLTIONS. All rights reserved
...
if( ! nv_function_exists( 'nv_block_pa_groups' ) )
{
	function nv_block_config_pa_groups( $module, $data_block, $lang_block )
	{
		global $site_mods;
	...
	function nv_block_config_pa_groups_submit( $module, $lang_block )
	{
		global $nv_Request;
		$return = array();
		$return['error'] = array();
	...
	function nv_block_pa_groups( $block_config )
	{
		...
		if( ! empty( $list ) )
		{
			if( file_exists( NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/news/block_pa_groups.tpl' ) )
			{
			...
			$xtpl = new XTemplate( 'block_pa_groups.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news' );
...
if( defined( 'NV_SYSTEM' ) )
{
	...
	if( isset( $site_mods[$module] ) )
	{
		...
		$content = nv_block_pa_groups( $block_config );
	}
}
