...
if( ! nv_function_exists( 'nv_block_news_cat_cal' ) )
{
	function nv_block_config_news_cat_cal( $module, $data_block, $lang_block )
	{
		global $site_mods;
		...
		$html .= '<tr>';
		$html .= '<td>' . $lang_block['numview'] . '</td>';
		$html .= '<td><input type="text" class="form-control w200" name="config_numview" size="5" value="' . $data_block['numview'] . '"/></td>';
		$html .= '</tr>';
	...
	function nv_block_config_news_cat_cal_submit( $module, $lang_block )
	{
		global $nv_Request;
		$return = array();
		$return['error'] = array();
		...
		$return['config']['numview'] = $nv_Request->get_int( 'config_numview', 'post', 0 );
	...
	function nv_block_news_cat_cal( $block_config )
	{
		...
		if( ! empty( $list ) )
		{
			if( file_exists( NV_ROOTDIR . '/themes/' . $global_config['module_theme']  . '/modules/news/block_newscat_cal.tpl' ) )
			{
			...
			$xtpl = new XTemplate( 'block_newscat_cal.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news' );
			...
			foreach( $list as $l )
			{
				...
				$xtpl->assign( 'ROW', $l );
				$xtpl->assign( 'TITLE1',  nv_clean60($l['title'], 40 ));
...
if( defined( 'NV_SYSTEM' ) )
{
	...
	if( isset( $site_mods[$module] ) )
	{
		...
		$content = nv_block_news_cat_cal( $block_config );
	}
}
