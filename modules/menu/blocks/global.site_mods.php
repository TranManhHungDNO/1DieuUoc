...
if( ! nv_function_exists( 'nv_menu_site_mods' ) )
{
	...
	function nv_menu_site_mods( $block_config )
	{
		...
		if( file_exists( NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/menu/global.pa_menu.tpl' ) )
		{
			$block_theme = $global_config['module_theme'];
		}
		elseif( file_exists( NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/menu/global.pa_menu.tpl' ) )
		{
		...
		$xtpl = new XTemplate( 'global.pa_menu.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/menu' );
		...
		foreach( $site_mods as $modname => $modvalues )
		{
			if( in_array( $modname, $block_config['module_in_menu'] ) AND ! empty( $modvalues['funcs'] ) )
			{
				...
				if( ! empty( $modvalues['funcs'] ) )
				{
					...
					elseif($modname =="about" )
					{

						$db->sqlreset()
							->select( 'title, alias' )
							->from( NV_PREFIXLANG . '_' . $modvalues['module_data']  )
							->where( 'status=1' )
							->order( 'weight ASC' )
							->limit( 10 );
						$list = nv_db_cache( $db->sql(), '', $modname );
					...
					if( $modvalues['module_file'] == 'shops'  or   $modvalues['module_file'] == 'products')
					{
						$db->sqlreset()
